...
import com.sanleng.mobilefighting.activity.VideoSurfaceActivity;

public class LightnessController {
	public static void turnDown(Context context, float yDilta, int heightPixels) { // 获取系统当前的亮度
		try {
			int current_brightness = Settings.System.getInt(context.getContentResolver(),
			...
			// 获取SeekBar
			SeekBar bright_display = ((VideoSurfaceActivity) context).getBright_display();
	...
	public static void turnUp(Context context, float yDilta, int heightPixels) { // 获取系统当前的亮度
		try {
			int current_brightness = Settings.System.getInt(context.getContentResolver(),
					Settings.System.SCREEN_BRIGHTNESS);
