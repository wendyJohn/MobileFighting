...
import com.sanleng.mobilefighting.adapter.FaultAdapter;
...
public class FaultFragment extends BaseFragment {
	...
	private void InitTextView() {
		...
		l_date2 = (LinearLayout) view.findViewById(R.id.l_date2);
	...
	private void loadData(int page) {
		...
		RequestUtils.ClientPost(URLs.FireAlarm_URL, params, new NetCallBack() {
			...
			public void onMySuccess(String result) {
				...
				System.out.println("故障数据请求成功" + result);
				try {
					int length = 10;
					int SIZE = 0;
					...
					if (msg.equals("获取成功")) {
						...
						if (scope.equals("oneday")) {
							data_text.setText("今日共有" + listsize + "起故障");
						}

						if (scope.equals("sevendays")) {
							data_text.setText("本周共有" + listsize + "起故障");
						}
						if (scope.equals("thirtydays")) {
							data_text.setText("本月共有" + listsize + "起故障");
						}
